                    ...
                    <article class="flex max-w-xl flex-col items-start justify-between">
                        ...
                        {{-- <section class="space-y-6">
                            <x-danger-button x-data=""
                                x-on:click.prevent="$dispatch('open-modal', 'confirm-user-deletion-{{ $post->id }}')">
                                {{ __('Delete post') }}</x-danger-button>
    
                            <x-modal :name="'confirm-user-deletion-' . $post->id" :show="$errors->userDeletion->isNotEmpty()" focusable>
                                <form method="post" action="{{ route('profile.deleteUser', $post->id) }}"
                                    class="p-6">
                                    <form method="post" action="{{ route('posts.destroy', $post->id) }}" class="p-6">
